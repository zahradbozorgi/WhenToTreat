...
class BaseGenModel(object, metaclass=BaseGenModelMeta):
    ...
    def __init__(self, w, t, y, train_prop=1, val_prop=0, test_prop=0,
                 ...
                 verbose=True, is_eventLog=True, train_indexes=[], val_indexes=[], test_indexes=[]):
        """
        ...
        if is_eventLog:
            # train_idxs = np.where(np.isin(w[:,w.shape[1]-1], train_indexes))
            # val_idxs = np.where(np.isin(w[:,w.shape[1]-1], val_indexes))
            # test_idxs = np.where(np.isin(w[:,w.shape[1]-1], test_indexes))
            train_idxs = train_indexes
            val_idxs = val_indexes
            test_idxs = test_indexes
        else:
