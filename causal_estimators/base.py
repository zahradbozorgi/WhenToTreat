...
#         batches_1 = np.array_split(t1, t1.shape[0] / 100)
    ...
    def estimate_CI_forest(self, t1=1, t0=0, w=None):
        w = self.w if w is None else w
        self._raise_exception_if_not_fitted()
        ...
        lower, upper = self.econml_estimator.predict_interval(batches[0])
        ii = 0
        for batch in batches[1:]:
            lb, ub = self.econml_estimator.predict_interval(batch)
            lower = np.append(lower, lb)
            upper = np.append(upper,ub)
            ii += 1
            # self.econml_estimator.effect(T0=t0, T1=t1, X=w)
        return lower, upper
    ...
    def ite_conf_int(self, t1=1, t0=0, w=None, percentile=.95):
        ...
        return self.econml_estimator.const_marginal_effect_interval(X=w)
