...
class ORthoforestDML(BaseEconMLEstimator):
    ...
    def __init__(self, outcome_models,
                 prop_score_model=LogisticRegression(),
                 n_trees = 5, max_depth = 10, subsample_ratio = 0.7, lambda_reg = 1,
        ...
        super().__init__(econml_estimator=DMLOrthoForest(min_leaf_size=min_leaf_size,
            model_Y=outcome_models, model_T=prop_score_model, model_Y_final=outcome_models,
            discrete_treatment=discrete_treatment,n_trees=n_trees, max_depth=max_depth,
                                                         subsample_ratio=subsample_ratio,
                                                         lambda_reg=lambda_reg))

    def estimate_CI(self, X):
        return self.ite_conf_int(w=X)
...
class CausalForest(BaseEconMLEstimator):
    ...
    def estimate_ate(self, w):
        return self.estimate_ate_forest(w=w)

    def estimate_ite(self, w):
        return self.estimate_ite_forest(w=w)

    def estimate_CI(self, w):
        return self.estimate_CI_forest(w=w)
