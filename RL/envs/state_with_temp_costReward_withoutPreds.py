...
class StatewithTempCostRewardwithoutPred(BaseEnv):
    ...
    def __init__(self):
        ...
        low_array = np.array([0., np.finfo(np.float32).min, np.finfo(np.float32).min]) #, 0,1,1,0,0,0
        high_array = np.array([1., np.finfo(np.float32).max, np.finfo(np.float32).max]) #np.finfo(np.float32).max, np.finfo(np.float32).max,12,6,23, np.finfo(np.float32).max
    ...
    def _create_state(self):
        ...
        self.state = np.array(
            [relative_position, self.lower, self.upper])
        return self.state
