...
def evaluate_directory(
    ...
    dataset_roots = [Path(i) for i in os.listdir(checkpoint_dir) if i != ".ipynb_checkpoints"]
    results = {}
    # For each overall dataset (LBIDD, lalonde, etc.)
    for root in dataset_roots:
        # print(root)
        subdatasets = [i for i in os.listdir(checkpoint_dir / root) if i != ".ipynb_checkpoints"]
        subdatasets = [subdatasets[0]]
        print(subdatasets)
        ...
        for subdata in subdatasets:
            ...
            if not subdata.startswith('.'):
                ...
                print(subdata_path)
                subfolders = [f.path for f in os.scandir(subdata_path) if f.is_dir() and f.name != ".ipynb_checkpoints"]
                ...
                ites, ate, w, t, y, train_ids, val_ids, test_ids = get_data(args)
...
if __name__ == "__main__":
    evaluate_directory(data_filter='bpic12Caise', num_tests=1, n_uni=None, n_multi=200)
